  ...
  return (
    <div className="min-h-screen bg-[#0a0a0f] text-[#f0f0f8] overflow-hidden font-sans">
      ...
      <div className="grid min-h-screen grid-cols-1 lg:grid-cols-[240px_1fr]">
        ...
        <aside
          className={[
            "z-50 w-[240px] bg-[#13131a] border-r border-[#1c1c27] flex flex-col",
            "fixed inset-y-0 left-0 transform transition-transform duration-300 ease-in-out lg:sticky lg:top-0 lg:translate-x-0",
          ...
          <div className="flex-1 overflow-y-auto py-4 custom-scrollbar">
            <nav className="space-y-1 px-3">
              {NAV_ITEMS.map((item) => {
                ...
                const active = dashboardActive ? location.pathname === "/admin" || location.pathname === "/" : location.pathname.startsWith(item.path);
                return (
                  <Link
                    key={item.path}
                    to={item.path}
